    ...
    function check(input, val, type) {
        ...
        if(type == "name" || type == "phone"){
            if(!checkInput(val, type)) {
                ...
                if(type == "name"){
                    errorFun("Имя должно содержать только буквы!");
                }else {
                    errorFun("Телефон введен неправильно!");
                }
                return false;
            }else  {
            ...
            for (let i = 0; i < contact.length; i++) {
                if(contact[i][key] == val) {
                    conditionInput(input, 'red');
                    if(type == "name"){
                        errorFun("Имя уже сущесвтует");
                    }else {
                        errorFun("Телефон уже существует!");
                    }
                    return false;
                }else  {
                    conditionInput(input, 'grey');
            ...
            if (val != "") {
                if (!checkInput(val, type)) {
                    conditionInput(input, 'red');
                    errorFun("Поиск должен содержать только буквы!");
                    return false;
                } else {
                    conditionInput(input, 'grey');
                }
...
//----------------------OnClick--------------------------------
    ...
    function closeContact(clicked_id) {
        for (let i = 0; i < contact.length; i++) {
            ...
            //--Empty contact
            if(contact.length == 0) {
                textBlock('Телефонная книга пуста!');
            }
        }
        //--null search
        valNull(search);
    ...
    btnAddContact.onclick = function(){
        ...
        //--Check empty Phone
        if(telValue === ""){
            errorFun("Телефон не должен быть пустым !");
        ...
        //--Check empty Name
        if(nameValue === ""){
            errorFun("Имя не должно быть пустым !");
            conditionInput(name, 'red');
        }else {
            //--Check length Name
            if(nameValue.length < 3){
                errorFun("Имя слишком короткое (минимум из 3 букв)!");
                conditionInput(name, 'red');
            }else if(nameValue.length > 10){
                errorFun("Имя слишком большое (максимум из 10 букв)!");
                conditionInput(name, 'red');
            }else {
                //--Check Name
